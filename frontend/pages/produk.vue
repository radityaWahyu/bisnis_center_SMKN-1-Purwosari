<template>
  <section>
    <div class="header">
      <div class="container">
        <h3 class="title">PRODUK SMK NEGERI 1 PURWOSARI</h3>
      </div>
    </div>
    <div class="content container">
      <div class="title">
        <p>
          Berisikan daftar produk - produk yang di produksi oleh setiap jurusan
      ...
      <b-message type="is-info" has-icon v-else-if="rowLength === 0">
        ...
        <p :style="{ fontSize: '14px' }">
          Maaf, Belum terdapat data <strong>PRODUK</strong> yang disimpan
          kedalam sistem.
        </p>
      </b-message>
      ...
      meta: [
        {
          hid: "description",
          name: "description",
          content: "Daftar Produk Siswa SMKN 1 Purwosari",
        },
      ],
    };
  },
  ...
  async fetch() {
    try {
      ...
      const product = await this.$store.dispatch(
        "product/getProduct",
        "type=produk&paginate=true&limit=8"
      );
  ...
  methods: {
    async changePage(data) {
      try {
        ...
        const product = await this.$store.dispatch(
          "product/getProduct",
          `type=produk&paginate=true&limit=8&page=${data}`
        );
        this.row = product.data;
        this.meta = product.meta;
        this.links = product.links;
