  ...
  <a-card id="datatable">
    ...
    >
      ...
      <span slot="action" class="btn-action" slot-scope="text, record">
        <button-publish :id="record.id" :data-best="record.publish" />
        <a-tooltip placement="right">
          ...
          <a-dropdown>
            ...
            <a-menu slot="overlay">
              <a-menu-item key="1">
                <nuxt-link
                  :to="{ name: 'berita-id', params: { id: record.id } }"
                >
              ...
              <a-menu-item key="2">
                <nuxt-link
                  :to="{ name: 'berita-detail-id', params: { id: record.id } }"
                >
...
export default {
  ...
  components: {
    buttonPublish: () => import("~/components/berita/buttonPublish")
  },
  created() {
    this.fetchData();
    this.getList();
  ...
  methods: {
    ...
    async fetchData() {
      ...
      try {
        ...
        const { data, meta } = await this.getPost(url);
        this.row = data;
    ...
    async setBest(id, value) {
      try {
        const hide = this.$message.loading("Update data produk/jasa", 0);
